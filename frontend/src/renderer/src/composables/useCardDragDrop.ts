...
export function useCardDragDrop(newCardForm: NewCardForm, handleCreateCard: () => void) {
  ...
  function onTypeDragStart(t: any) {
    try {
      ;(event as DragEvent).dataTransfer?.setData('application/x-card-type-id', String(t.id))
    } catch {}
  }
  ...
  async function onCardsPaneDrop(e: DragEvent) {
    try {
      ...
      if (freeCardId) {
        await copyCard(Number(freeCardId), {
          target_project_id: projectStore.currentProject!.id,
  ...
  async function onTypesPaneDrop(e: DragEvent) {
    try {
      ...
      if (!effective) {
        ElMessage.warning('该卡片暂无可用结构，无法生成类型')
        return
      }
      ...
      await createCardType({
        name: finalName,
        description: `${finalName}的默认卡片类型`,
        json_schema: effective
      } as any)
  ...
  async function handleNodeDrop(
    ...
    try {
      ...
      if (targetCard.__isGroup && dropType === 'inner') {
        ...
        const maxOrder =
          rootCards.length > 0 ? Math.max(...rootCards.map((c) => c.display_order || 0)) : -1

        await cardStore.modifyCard(
          draggedCard.id,
          { parent_id: null, display_order: maxOrder + 1 },
          { skipHooks: true }
        )
      ...
      if (dropType === 'inner') {
        ...
        const maxOrder =
          children.length > 0 ? Math.max(...children.map((c) => c.display_order || 0)) : -1

        await cardStore.modifyCard(
          draggedCard.id,
          { parent_id: targetCard.id, display_order: maxOrder + 1 },
          { skipHooks: true }
        )
  ...
  async function onExternalDropToNode(e: DragEvent, nodeData: any) {
    ...
    if (Number.isFinite(typeId) && typeId > 0) {
      ...
      const newCard = await cardStore.addCard({
        title: '新建卡片',
        card_type_id: typeId,
